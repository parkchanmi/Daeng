		...
		<div id="contents_qna">
			<div class="board_qna">
				...
				<div class="write_area_qna">
					<form method="post" name="writeform_qna"
						action="/DaengDaeng/board/qna/writePro.do" onsubmit="summer();">
						...
						<input type="hidden" name="board_type" value="Q&A">
						<ul>
							
							<li class="write_writer_qna"><label>작성자</label>
								<h3>${memberDTO.mem_name}</h3></li>
							<li class="write_tit_qna"><label>제 목</label>
							<c:if test="${num == 0}">
							<h3><input type="text" size="40" maxlength="50" name="board_title"></h3> 
							</c:if>
							<c:if test="${num != 0}">
							<h3>	<input type="text" size="40" maxlength="50" name="board_title" value="[답변]"></h3> 
							</c:if>
							</li>
						</ul>
						<div class="write_box_qna">
							<div class="write_box_qna">
								<div id="summernote">글을 작성하세요</div>
							...
							<div class="btn_write_qna">
								<input type="submit" value="글쓰기"> <input type="reset"
									...
									OnClick="window.location='/DaengDaeng/board/qna/qna_list.do?pageNum=${pageNum}'">
							</div>
						</div>
					</form>
				</div>
