		...
		<div id="contents_qna">
			<div class="board_list_qna">
				...
				<div class="write_btn_qna">
					<a href="/DaengDaeng/board/lost/owner/owner_writeForm.do">글쓰기</a>
				</div>
				<c:if test="${count == 0}">
				...
				<c:if test="${count > 0}">
					<div class="lost_box">
						<c:forEach var="article" items="${articleList}">
							<dl class="lost_list">
								<a href="/DaengDaeng/board/lost/owner/owner_contents.do?num=${article.lost_code}&pageNum=${currentPage}">
									<dt>
										<img src="/DaengDaeng/images/dogimg.png" height="200">
									</dt>
									<dd>
										<h3>작성자 :</h3>
				...
				<c:if test="${count > 0}">
					<table width="990" cellpadding="0" cellspacing="0" align="center">
						<thead style="background-color:#EE8300;  color:#fff;">	
							<tr height="30">
								<th align="center" width="50">번호</td>
						...
						<c:forEach var="article" items="${articleList}">
							<tr height="30">
								...
								<td width="250"><a
									href="/DaengDaeng/board/lost/owner/owner_contents.do?num=${article.lost_code}&pageNum=${currentPage}">${article.lost_code}</a></td>
				...
				<div class="paging_qna">
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href=/DaengDaeng/board/lost/owner_list.do?pageNum=${startPage-10 }">[이전]</a>
						</c:if>

						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<div class="page_num_qna">
								<a href="/DaengDaeng/board/lost/owner_list.do?pageNum=${i}">${i}</a>
							</div>
						</c:forEach>

						<c:if test="${endPage < pageCount}">
							<a
								href="/DaengDaeng/board/lost/owner_list.do?pageNum=${startPage + 10}">[다음]</a>
						</c:if>
					</c:if>
				...
				<div class="search">
					<form method="post" name="search"
						action="/DaengDaeng/board/lost/owner_list.do">
						<select name="searchn">
