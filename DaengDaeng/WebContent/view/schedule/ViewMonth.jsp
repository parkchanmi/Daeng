						...
						<TABLE cellpadding="0" cellspacing="0" border="0"
							...
							<TR>
								<TD align="right" width="365"><A
									href="/DaengDaeng/view/schedule/View.jsp?type=MONTH&curYear=<c:out value="${curYear}"/>&curMonth=<c:out value="${curMonth-1}"/>&curDay=<c:out value="${curDay}"/>">◀</a>
													...
													<c:out value="${currentDay}"/>
																				...
																				<TR>
																					<TD height="68" width="70" valign="top">
																						<TABLE>
																							<c:forEach var="dayIndex"
																								...
																								<c:if test="${currentDay == db_day}">
																									<TR>
																										<TD><A
																											href="javascript:view('${dayIndex.msch_schcode}')">
																												${dayIndex.sch_title} </A> <c:set var="dbIndex"
																												value='${dbIndex + 1}' /></TD>
																									</TR>
																								</c:if>
																							</c:forEach>
																						...
																						<TABLE>
																							<c:forEach var="dayIndex2"
																								items="${club_schedule_query.rows}">
																								<fmt:formatDate var="db_day"
																									value="${dayIndex2.sch_sdate}" pattern="d" />
																								<c:if test="${currentDay == db_day}">
																									<TR>
																										<TD><A
																											href="javascript:view('${dayIndex2.csch_schcode}','${dayIndex2.csch_memcode==memberDTO.mem_code}')">
																												${dayIndex2.ci_name}:${dayIndex2.sch_title}
																										</A> <c:set var="dbIndex" value='${dbIndex + 1}' />
																										</TD>
																									</TR>
																								</c:if>
																							</c:forEach>
...
<SCRIPT type="text/javascript">
	function view(str, ok) {
		OpenWin("Update.jsp?type=SELECT&sch_code=" + str + "&update_ok=" + ok,
				480, 360);
	}

	function dWrite(y, m, d) {
		OpenWin("Update.jsp?curYear=" + y + "&curMonth=" + m + "&curDay=" + d,
				470, 320);
	}
