...
<sql:query var="month_query" dataSource="${dataSource}" scope="session">
select * from member_schedule, schedule where MEMBER_SCHEDULE.MSCH_SCHCODE=schedule.sch_code and msch_memcode=?
	 and to_char(sch_sdate,'mm') >= ? and to_char(sch_sdate,'mm') < ? ORDER BY sch_sdate
...
<sql:query var="club_schedule_query" dataSource="${dataSource}" scope="session">
select csch_cicode,csch_schcode, csch_memcode, sch_title, sch_contents, sch_sdate, ci_name 
from club_schedule,club_join,schedule,club_info 
where club_schedule.csch_cicode=club_join.cj_cicode 
and club_join.cj_cicode=club_info.ci_code 
and club_schedule.csch_schcode=schedule.sch_code 
and cj_memcode=?
	<sql:param value="${memberDTO.mem_code}"/>
</sql:query>
...
<c:set var="action_target" 	value="/DaengDaeng/view/schedule/ViewMonth.jsp"/>	
