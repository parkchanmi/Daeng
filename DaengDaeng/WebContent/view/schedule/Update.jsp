...
<c:set var="update_ok" value="${param.update_ok}"/>
...
<c:choose>
	...
	<script>alert('update');</script>
		...
		<SCRIPT LANGUAGE="JavaScript">
			opener.location.href='/DaengDaeng/view/schedule/View.jsp?curYear=<c:out value="${curYear}"/>&curMonth=<c:out value="${curMonth}"/>&curDay=<c:out value="${curDay}"/>';
			self.close();
		</SCRIPT>
	</c:when>
	<%-- INSERT --%>
	...
	<script>alert('insert');</script>
		...
		<c:if test="${sch_kind=='member'}">
			<sql:update var="insert"  dataSource="${dataSource}">
				INSERT  INTO member_schedule values(mem_schedule_sequence.nextval,schedule_sequence.nextval-1,?)
		<sql:param value="${memberDTO.mem_code}"/>
			</sql:update>
		</c:if>
...
<FORM METHOD=POST NAME='theForm' ACTION='/DaengDaeng/view/schedule/UpdateForm.jsp'>
	<INPUT TYPE="hidden" NAME='type'  VALUE='<c:out value="${type}"/>'>
	<INPUT TYPE="hidden" NAME='update_ok'  VALUE='<c:out value="${update_ok}"/>'>
