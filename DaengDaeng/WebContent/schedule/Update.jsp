...
<c:choose>
	...
	<c:when test="${type == 'SELECT'}">		
		<sql:query var="select" dataSource="${dataSource}">
			SELECT * FROM tb_schedule WHERE schedule_id=?
	...
	<c:when test="${type == 'UPDATE'}">
		<sql:update var="update" dataSource="${dataSource}">
			UPDATE tb_schedule SET schedule_date_time=?, schedule_subject=?, schedule_content=? WHERE schedule_id=?
	...
	<c:when test="${type == 'INSERT'}">
		<sql:query var="nextval" dataSource="${dataSource}">
			select (max(schedule_id)+1) as schedule_id from tb_schedule
		</sql:query>
